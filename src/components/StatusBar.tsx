    ...
    <footer className="bg-slate-950 border-t border-slate-800/80 px-4 py-2.5 text-slate-300 text-xs flex flex-wrap items-center justify-between gap-4 sticky bottom-0 z-20 shadow-2xl backdrop-blur-md">
      ...
      <div className="flex flex-wrap items-center gap-4 sm:gap-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 font-mono text-[11px] bg-slate-900 px-2.5 py-1 rounded-lg border border-slate-800">
            <span className="text-amber-400 font-semibold">
              {stats.original.tokens.toLocaleString("fr-FR")} <span className="text-[10px] text-slate-500 font-sans">tokens</span>
            </span>
            <span className="text-slate-600">•</span>
            <span className="text-slate-300">
              {stats.original.words.toLocaleString("fr-FR")} <span className="text-[10px] text-slate-500 font-sans">mots</span>
            </span>
            <span className="text-slate-600">•</span>
            <span className="text-slate-400">
              {stats.original.characters.toLocaleString("fr-FR")} <span className="text-[10px] text-slate-500 font-sans">car.</span>
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-400 font-medium">Compressé (Caveman):</span>
          <div className="flex items-center gap-2 font-mono text-[11px] bg-slate-900 px-2.5 py-1 rounded-lg border border-slate-800">
            <span className="text-emerald-400 font-bold">
              {stats.compressed.tokens.toLocaleString("fr-FR")} <span className="text-[10px] text-slate-500 font-sans">tokens</span>
            </span>
            <span className="text-slate-600">•</span>
            <span className="text-slate-200">
              {stats.compressed.words.toLocaleString("fr-FR")} <span className="text-[10px] text-slate-500 font-sans">mots</span>
            </span>
            <span className="text-slate-600">•</span>
            <span className="text-slate-300">
              {stats.compressed.characters.toLocaleString("fr-FR")} <span className="text-[10px] text-slate-500 font-sans">car.</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {apiUsage && (
          <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 px-3 py-1 rounded-xl text-amber-300">
            ...
            <div className="flex items-baseline gap-1 text-[11px]">
              <span className="text-slate-400 font-medium hidden sm:inline">Coût LLM:</span>
              <span className="font-mono font-bold text-amber-400">
                {apiUsage.totalTokens.toLocaleString("fr-FR")} tokens
              </span>
        ...
        <button
          ...
          title="Copier le texte compressé"
        >
          {copied ? (
            <>
        ...
        {hasCompressed && (
          <button
            ...
            title="Télécharger le résultat au format Markdown"
          >
